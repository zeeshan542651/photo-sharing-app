...
export interface IStorage {
  ...
  getPhoto(
    id: number,
    userId?: number
  ): Promise<PhotoDetailWithRatings | undefined>;
  createPhoto(photo: InsertPhoto & { userId: number }): Promise<Photo>;

  // Comments & Ratings
  createComment(
    comment: InsertComment & { userId: number; photoId: number }
  ): Promise<Comment>;
  createOrUpdateRating(
    rating: InsertRating & { userId: number; photoId: number }
  ): Promise<Rating>;
...
export class DatabaseStorage implements IStorage {
  ...
  async getPhotos(
    ...
    if (search) {
      query.where(
        sql`${photos.title} ILIKE ${`%${search}%`} OR ${
          photos.location
        } ILIKE ${`%${search}%`}`
      );
    }
  ...
  async createOrUpdateRating(
    ...
    const [existing] = await db
      ...
      .where(
        and(
          eq(ratings.photoId, rating.photoId),
