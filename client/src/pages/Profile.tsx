...
export default function Profile() {
  ...
  const updateProfile = useMutation({
    mutationFn: async (
      data: z.infer<typeof profileSchema> & { profilePicture?: string }
    ) => {
  ...
  const totalRatings = userPhotos.reduce(
    (acc, p) => acc + (p.averageRating ? 1 : 0),
    0
  );
  const avgRating =
    totalRatings > 0
      ? (
          userPhotos.reduce(
            (acc, p) => acc + (parseFloat(String(p.averageRating)) || 0),
            0
          ) / totalRatings
        ).toFixed(1)
      : "0.0";
          ...
          {isCreator ? (
            ...
            <section className="text-center py-12">
              <div className="max-w-md mx-auto">
                ...
                <p className="text-muted-foreground mb-6">
                  Explore amazing photos from our creative community. Rate your
                  favorites and join the conversation!
                </p>
                <Button
                  onClick={() => setLocation("/")}
                  size="lg"
